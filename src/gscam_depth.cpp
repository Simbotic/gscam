...
namespace gscam_depth {
  ...
  bool GSCam::configure()
  {
    ...
    nh_private_.param("image_encoding", image_encoding_, sensor_msgs::image_encodings::RGBA8);
    if (image_encoding_ != sensor_msgs::image_encodings::RGBA8 &&
  ...
  bool GSCam::init_stream()
  {
    ...
    if (image_encoding_ == "jpeg") {
        jpeg_pub_ = nh_.advertise<sensor_msgs::CompressedImage>("camera/depth/image_raw/compressed",1);
        cinfo_pub_ = nh_.advertise<sensor_msgs::CameraInfo>("camera/depth/camera_info",1);
    } else {
        camera_pub_ = image_transport_.advertiseCamera("camera/depth/image_raw", 1);
    }
